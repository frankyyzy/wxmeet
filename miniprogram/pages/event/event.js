...
Page({
  ...
  onLoad: function(options) {
    ...
    if (options.share) {
      wx.navigateTo({
        url:
          "/pages/selectTime/selectTime?share=true&eventId=" +
          options.eventId +
          "&eventName=" +
          options.eventName +
          "&createTime=" +
          options.createTime +
          "&datesArr=" +
          options.datesArr //  otherwise go to selectTime
      });
    }
    this.setData({
  ...
  setcolor: function(NumOfPeople) {
    //initialize 2d array
    var arr = new Array(this.data.times.length)
      .fill(0)
      .map(() => new Array(this.data.times[0].length).fill(0));

    for (var i = 0; i < this.data.times.length; i++) {
      for (var j = 0; j < this.data.times[0].length; j++) {
        arr[i][j] =
          "rgba(0, 151, 19," + this.data.times[i][j] / NumOfPeople + ")";
      }
    }
    this.setData({
      color: arr
  ...
  onShareAppMessage: function() {
    ...
    return {
      ...
      path:
        "/pages/loading/loading?share=true&eventId=" +
        that.data.eventId +
        "&sponserId=" +
        that.data.sponser +
        "&eventName=" +
        that.data.eventName +
        "&createTime=" +
        that.data.createDate +
        "&datesArr=" +
        JSON.stringify(that.data.dates)
    };
  },
  /**
  ...
  onTouchStart: function(e) {
    ...
    this.setData({
      ...
      display:
        this.data.times[i][j].toString() +
        (this.data.times[i][j] == 1 ? " person " : " people ") +
        "available on " +
        this.data.dates[j] +
        " between " +
        i.toString() +
  ...
  onEditTap: function() {
    ...
    wx.navigateTo({
      url:
        "/pages/selectTime/selectTime?eventId=" +
        that.data.eventId +
        "&eventName=" +
        that.data.eventName +
        "&datesArr=" +
